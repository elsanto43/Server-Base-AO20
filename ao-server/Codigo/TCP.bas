...
Sub CloseSocket(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        If .Pos.map = Bestia_Map Then
            Call WarpUserChar(Userindex, bestia_tp.map, bestia_tp.x, bestia_tp.y, True)
        End If
            
        'torneos nVSn. usuario desconecta
        If .userTorneo.EnTorneo = True Then
            Call mod_Torneos.torneo_Muere(Userindex, True)
        End If
    ...
    With UserList(Userindex)
      ...
      '      .flags.Seguro = False
       ...
       ' End If
        ...
        'Si no tienen interes en usarlo pueden desactivarlo en el Server.ini
        If ConexionAPI Then
            Call ApiEndpointSendUserConnectedMessageDiscord(Name, .Desc, esLegion(Userindex), ListaClases(.Clase))
        End If
