...
Sub UpdateUserToDatabase(ByVal Userindex As Integer, _
    ...
    With UserList(Userindex)
        ...
        query = query & "neutrales_matados = " & .Faccion.CriminalesMatados & ", "
        query = query & "faccion = " & .Faccion.Bando & ", "
        query = query & "guild_index = " & .GuildIndex
        If Torneo.Activo And .userTorneo.EnTorneo = True Then
            query = query & ", lasttorneo_id = " & Torneo.uniqueid & ", "
            query = query & "torneo_data = " & get_torneodata(Userindex) & " "
        Else
            query = query & " "
        End If
...
Sub LoadUserFromDatabase(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        Call CheckTorneoData(Userindex, Database_RecordSet!lasttorneo_id, Database_RecordSet!torneo_data)
        ...
        .Faccion.Bando = Database_RecordSet!Faccion
        ...
        .Faccion.NeutralesMatados = Database_RecordSet!neutrales_matados
...
End Sub
Public Sub SumGoldtoUserDB(ByVal UserName As String, ByVal addValue As Long)
    ...
    query = "UPDATE user SET Gold=Gold+" & addValue & " WHERE UPPER(name) = '" & UCase$(UserName) & "';"
...
ErrorHandler:
    Call LogDatabaseError("Error in SumGoldtoUserDB: " & UserName & ". " & Err.Number & " - " & Err.description)
...
Public Function UserEsNeutroDatabase(ByVal UserName As String) As Boolean
    ...
    query = "SELECT bando FROM user WHERE UPPER(name) = '" & UCase$(UserName) & "' AND deleted = FALSE;"

    Set Database_RecordSet = Database_Connection.Execute(query)

    If Database_RecordSet.BOF Or Database_RecordSet.EOF Then
        UserEsNeutroDatabase = True
        Exit Function

    End If

    UserEsNeutroDatabase = (CByte(Database_RecordSet!Bando) = eFaccion.Neutral)
    Set Database_RecordSet = Nothing
    Call Database_Close
...
ErrorHandler:
    Call LogDatabaseError("Error in UserEsArmadaDatabase: " & UserName & ". " & Err.Number & " - " & Err.description)

End Function

Public Function UserEsArmadaDatabase(ByVal UserName As String) As Boolean
    ...
    If Database_RecordSet.BOF Or Database_RecordSet.EOF Then
        UserEsArmadaDatabase = False
        Exit Function

    End If

    UserEsArmadaDatabase = (CByte(Database_RecordSet!Bando) = eFaccion.Armada)
    Set Database_RecordSet = Nothing
    Call Database_Close
...
Public Function UserEsLegionDatabase(ByVal UserName As String) As Boolean
    ...
    If Database_RecordSet.BOF Or Database_RecordSet.EOF Then
        UserEsLegionDatabase = False
        Exit Function

    End If

    UserEsLegionDatabase = (CByte(Database_RecordSet!Bando) = eFaccion.Legion)
    Set Database_RecordSet = Nothing
    Call Database_Close
...
ErrorHandler:
    Call LogDatabaseError("Error in UserEsLegionDatabase: " & UserName & ". " & Err.Number & " - " & Err.description)
...
Public Function GetUserGoldFromDatabase(ByVal UserName As String) As Long
    ...
    If PersonajeExiste(UserName) Then
        ...
        query = "SELECT Gold FROM user WHERE UPPER(name) = '" & UCase$(UserName) & "';"
       ...
       ' End If
        GetUserGoldFromDatabase = Database_RecordSet!Gold
    ...
    Exit Function
ErrorHandler:
    Call LogDatabaseError("Error in getUsergoldFromDatabase: " & UserName & ". " & Err.Number & " - " & Err.description)
