        ...
        If PuedeLanzar(Userindex, SpellIndex) Then
            ...
            Select Case Hechizos(SpellIndex).Target
                ...
                Case TargetType.uTerreno
                    If Hechizos(SpellIndex).tiempoCasteo = 0 Then
                        Call HandleHechizoTerreno(Userindex, SpellIndex)
                    Else
                        .CasteoIndex = SpellIndex
                        .CasteoSegundos = Hechizos(SpellIndex).tiempoCasteo
                        .targetMapBCK.map = .flags.TargetMap
                        .targetMapBCK.x = .flags.TargetX
                        .targetMapBCK.y = .flags.TargetY
                        
                        Call SendData(SendTarget.ToPCArea, Userindex, prepareMessageBarraCasteo(UserList(Userindex).Char.CharIndex, .CasteoSegundos))
                        
                        Call WriteConsoleMsg(Userindex, "Te estás concentrando para lanzar el hechizo... No te muevas", FontTypeNames.FONTTYPE_INFO)
                    End If
...
Public Function CanSupportNpc(ByVal CasterIndex As Integer, _
    ...
    With UserList(CasterIndex)
        ...
        ElseIf esLegion(OwnerIndex) Then
