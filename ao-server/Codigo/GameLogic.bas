    ...
    If InMapBounds(map, x, y) Then
        ...
        With MapData(map, x, y)
            ...
            If .TileExit.map > 0 And .TileExit.map <= NumMaps Then
                ...
                ElseIf DestPos.map = mod_EventoBestia.Bestia_Map Then
                    If bestia_PuedeEntrar(Userindex) Then
                        If LegalPos(DestPos.map, DestPos.x, DestPos.y, PuedeAtravesarAgua(Userindex)) Then
                            Call WarpUserChar(Userindex, DestPos.map, DestPos.x, DestPos.y, FxFlag)
                        Else
                            Call ClosestLegalPos(DestPos, nPos)
                        ...
                        End If
                    Else 'Ya entraron 2 partys
                        Call WriteConsoleMsg(Userindex, "Ya entraron 2 partys validas al dungeon, no puedes entrar.", FontTypeNames.FONTTYPE_INFO)
                        Call ClosestStablePos(UserList(Userindex).Pos, nPos)
                ...
                ElseIf MapInfo(DestPos.map).Restringir = eRestrict.restrict_faccion Then 'Es mapa de faccionarios?
                    ...
                    If esArmada(Userindex) Or esLegion(Userindex) Or EsGm(Userindex) Then
                        If LegalPos(DestPos.map, DestPos.x, DestPos.y, PuedeAtravesarAgua(Userindex)) Then
                            Call WarpUserChar(Userindex, DestPos.map, DestPos.x, DestPos.y, FxFlag)
                        Else
                            Call ClosestLegalPos(DestPos, nPos)
                ...
                If UserList(TempCharIndex).flags.AdminInvisible = 0 Or .flags.Privilegios And PlayerType.Dios Then
                    ...
                    With UserList(TempCharIndex)
                        ...
                        If LenB(.DescRM) = 0 And .showName Then 'No tiene descRM y quiere que se vea su nombre.
                            ...
                            If .Faccion.Bando = eFaccion.Armada And .Faccion.Jerarquia > 0 Then
                                Stat = Stat & " <Ejercito Real> " & "<" & TituloReal(TempCharIndex) & ">"
                            ElseIf .Faccion.Bando = eFaccion.Legion And .Faccion.Jerarquia > 0 Then
                            ...
                            Else
                                ...
                                ElseIf UserList(TempCharIndex).Faccion.Bando = eFaccion.Legion Then
                                    Stat = Stat & "<" & Faccion(eFaccion.Legion).TituloNoEnlistado & ">" ' " <CRIMINAL>"
                                    ft = FontTypeNames.FONTTYPE_CRIMINAL
                                Else
                                     Stat = Stat & "<" & Faccion(eFaccion.Armada).TituloNoEnlistado & ">"
                    ...
                    If Len(Npclist(TempCharIndex).Desc) > 1 Then
                        ...
                        If Npclist(TempCharIndex).NPCtype = eNPCType.Noble Then
                            If Npclist(TempCharIndex).flags.Faccion = 0 Then 'Es el Rey.
                                ...
                                If UserList(Userindex).Faccion.Bando = eFaccion.Legion Then
                           ...
                           '         Else 'Es criminal enlistable.
                            ...
                            Else 'Es el demonio
                                ...
                                If UserList(Userindex).Faccion.Bando = eFaccion.Armada Then
