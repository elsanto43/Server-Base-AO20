...
Public Function HandleIncomingData(ByVal Userindex As Integer) As Boolean
    ...
    Select Case packetID
        Case ClientPacketID.ElegirFaccion
            Call handleElegirFaccion(Userindex)
            
        Case ClientPacketID.IniciarSubasta
            Call handleIniciarSubasta(Userindex)
            
        Case ClientPacketID.SubastaOfrecer
            Call handleSubastaOfrecer(Userindex)
        
        Case ClientPacketID.doViajar
            Call handleDoViajar(Userindex)
            
         Case ClientPacketID.CanjearItem
            Call handleCanjearItem(Userindex)
            
        Case ClientPacketID.requestCanjes
            Call handleRequestCanjes(Userindex)
        ...
        Case ClientPacketID.AdminTorneo
            Call handleAdminTorneo(Userindex)
    ...
    With UserList(Userindex)
        ...
        If .flags.Paralizado = 0 Then
            ...
            Else
                ...
                If .CasteoSegundos > 0 Then
                    Call WriteConsoleMsg(Userindex, "Se ha cancelado el hechizo", FontTypeNames.FONTTYPE_INFO)
                    Call SendData(SendTarget.ToPCArea, Userindex, prepareMessageBarraCasteo(UserList(Userindex).Char.CharIndex, .CasteoSegundos))
                    .CasteoSegundos = 0
                End If
            End If
...
Private Sub HandleLeaveFaction(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        Else
         ...
         '   If (TalkToDemon And .Faccion.Bando = eFaccion.Legion) Or (TalkToKing And Not criminal(Userindex)) Then 'Si se pueden unir a la faccion (status), son invitados
...
Private Sub HandleGuildFundate(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        'aca hacemos todos los chequeos necesarios.
        If modGuilds.PuedeFundarUnClan(Userindex, Error) Then
            Call WriteShowGuildFundationForm(Userindex)
        Else
            Call WriteConsoleMsg(Userindex, Error, FontTypeNames.FONTTYPE_INFOBOLD)
        End If
    End With
    ...
    With UserList(Userindex)
        ...
        For i = 1 To LastUser
            ...
            If UserList(i).ConnID <> -1 Then
                If UserList(i).Faccion.bando = eFaccion.Armada Then
...
Private Sub HandleOnlineChaosLegion(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        For i = 1 To LastUser
            ...
            If UserList(i).ConnID <> -1 Then
                If UserList(i).Faccion.bando = eFaccion.Legion Then
                    If UserList(i).flags.Privilegios And priv Then
                        list = list & UserList(i).Name & ", "
...
Public Sub WriteShowForumForm(ByVal Userindex As Integer)
    ...
    With UserList(Userindex)
        ...
        If esLegion(Userindex) Or EsGm(Userindex) Then
...
Public Function PrepareMessageCharacterDefendAnim(ByVal CharIndex As Integer) As String
    ...
    '***************************************************
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.PlayDefendAnim)
        Call .WriteInteger(CharIndex)
    
        PrepareMessageCharacterDefendAnim = .ReadASCIIStringFixed(.Length)
...
Private Sub handleEntrarTorneo(ByVal Userindex As Integer)
    With UserList(Userindex)
        Call .incomingData.ReadByte
        
        Call mod_Torneos.IngresaTorneo(Userindex)
    End With
End Sub

Private Sub handleAdminTorneo(ByVal Userindex As Integer)

    Dim tipoTorneo As Byte, Accion As Byte, numTeams As Byte
    ...
    With UserList(Userindex)
        ...
        numTeams = .incomingData.ReadByte   'numero de equipos para el torneo, 8, 16, 32 o 64
        tipoTorneo = .incomingData.ReadByte 'cantidad de users x equpo
        
        If Accion = 1 Then
            Call mod_Torneos.CrearTorneo(tipoTorneo, numTeams)
        ElseIf Accion = 2 Then
            ' cancelar torneo
            Call mod_Torneos.cancelar_Torneo
...
Public Sub writeSendCanjes(ByVal ID As Integer)
    With UserList(ID).outgoingData
        Dim i As Long
        
        Call .WriteByte(ServerPacketID.sendCanjes)
        
        Call .WriteInteger(numCanjes)
        
        For i = 1 To numCanjes
            Call .WriteASCIIString(ObjData(Canjes(i).recompensaItem.ObjIndex).Name & " (" & Canjes(i).recompensaItem.amount & " unidades)")
            
            Call .WriteByte(Canjes(i).pideTrofeosOro)
            Call .WriteByte(Canjes(i).pideTrofeosPlata)
            Call .WriteByte(Canjes(i).pideTrofeosBronce)
            
            Call .WriteInteger(ObjData(Canjes(i).recompensaItem.ObjIndex).GrhIndex)
            
        Next i
    End With
End Sub

Private Sub handleCanjearItem(ByVal ID As Integer)
    Call UserList(ID).incomingData.ReadByte ' packetid
    
    Dim iCanje As Integer
    iCanje = UserList(ID).incomingData.ReadInteger
    
    Call mod_Canjes.CanjearItem(iCanje, ID)
    
End Sub

Private Sub handleRequestCanjes(ByVal ID As Integer)
    Call UserList(ID).incomingData.ReadByte ' packetid
    Call writeSendCanjes(ID)
End Sub

Public Sub CargarDestinos()
    Dim nPath As String, Reader As clsIniManager, i As Long, wpos As String
    
    Set Reader = New clsIniManager
    
    nPath = DatPath & "Destinos.dat"
    
    Call Reader.Initialize(nPath)
    
    numDestinos = Reader.GetValue("INIT", "NumDestinos")
    
    For i = 1 To numDestinos
        With Destino(i)
            .Nombre = Reader.GetValue("DESTINO" & i, "Nombre")
            
            wpos = Reader.GetValue("DESTINO" & i, "WorldPos")
            
            .Destino.Map = val(ReadField(1, wpos, "-"))
            .Destino.X = val(ReadField(2, wpos, "-"))
            .Destino.Y = val(ReadField(3, wpos, "-"))
...
Private Sub handleDoViajar(ByVal ID As Integer)
Dim nDestino As Byte, NpcIndex As Integer
    
    With UserList(ID)
        Call .incomingData.ReadByte
        
        nDestino = .incomingData.ReadByte
        ...
        If Distancia(.Pos, Npclist(NpcIndex).Pos) > 5 Then
            Call WriteConsoleMsg(ID, "Estas demasiado lejos.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        
        If Npclist(NpcIndex).NPCtype = eNPCType.Viajante Then
            Call WarpUserChar(ID, Destino(nDestino).Destino.Map, Destino(nDestino).Destino.X, Destino(nDestino).Destino.Y, True)
            Call WriteConsoleMsg(ID, "Has viajado al destino", FontTypeNames.FONTTYPE_INFO)
        End If
        
    End With
End Sub

Private Sub handleIniciarSubasta(ByVal ui As Integer)
    Dim Slot As Byte, amount As Integer, ofertaInicial As Long
    With UserList(ui)
        Call .incomingData.ReadByte
        Slot = .incomingData.ReadByte
        amount = .incomingData.ReadInteger
        ofertaInicial = .incomingData.ReadLong
        
        Call mod_Subasta.IniciaSubasta(ui, .incomingData.ReadByte, .incomingData.ReadInteger, .incomingData.ReadLong)
        
    End With
End Sub

Private Sub handleSubastaOfrecer(ByVal ui As Integer)

    
End Sub
Public Sub writeViajesForm(ByVal ui As Integer)
    Call UserList(ui).outgoingData.WriteByte(ServerPacketID.ViajesForm)
End Sub

Public Sub writeAbrirSubasta(ByVal ui As Integer)
    Call UserList(ui).outgoingData.WriteByte(ServerPacketID.AbrirSubasta)
End Sub

Public Function prepareMessageBarraCasteo(ByVal CharIndex As Integer, ByVal max As Byte) As String
    ...
    '***************************************************
    With auxiliarBuffer
        Call .WriteByte(ServerPacketID.BarraCasteo)
        Call .WriteInteger(CharIndex)
        Call .WriteByte(max)
        
        prepareMessageBarraCasteo = .ReadASCIIStringFixed(.Length)

    End With

End Function

Private Sub handleElegirFaccion(ByVal ui As Integer)
    On Error Resume Next
    Dim tBando As Byte
    Dim tStr As String
    With UserList(ui)
        Call .incomingData.ReadByte
        tBando = .incomingData.ReadByte
        
        If .Faccion.lastFaccion <> eFaccion.Neutral Then
            Call WriteConsoleMsg(ui, "Debes ir hablar con el dios Kratos, el decidira si podras ingresar a la faccion.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        
        If .Faccion.bando <> eFaccion.Neutral And .Faccion.bando <> tBando Then
            Call WriteConsoleMsg(ui, "Primero debes salir de la faccion en la que estas ahora, y pedir el perdon faccionario.", FontTypeNames.FONTTYPE_INFO)
            Exit Sub
        End If
        
        .Faccion.bando = tBando
        If tBando = eFaccion.Armada Then
            tStr = "Para subir a la 1ra jerarqu�a deber�s matar " & Faccion(tBando).Jerarquia(1).LegionesMatados & " miembros de la Horda"
        ElseIf tBando = eFaccion.Legion Then
            tStr = "Para subir a la 1ra jerarqu�a deber�s matar " & Faccion(tBando).Jerarquia(1).ArmadasMatados & " miembros de la Armada"
        End If
        
        RefreshCharStatus ui
        Call WriteConsoleMsg(ui, "Has ingresado a la " & IIf(tBando = eFaccion.Armada, " Armada Imperial. ", " Horda Infernal. ") & tStr, FontTypeNames.FONTTYPE_GUILD)
