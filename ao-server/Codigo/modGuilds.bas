    ...
    If PersonajeExiste(Personaje) Then
        ...
        Select Case guilds(GuildIndex).Alineacion
            ...
            Case ALINEACION_GUILD.ALINEACION_ARMADA
                m_EstadoPermiteEntrarChar = UserEsArmada(Personaje)
                
            Case ALINEACION_GUILD.ALINEACION_NEUTRO
                m_EstadoPermiteEntrarChar = UserEsNeutral(Personaje)

            Case ALINEACION_GUILD.ALINEACION_LEGION
                m_EstadoPermiteEntrarChar = UserEsLegion(Personaje)
...
Private Function m_EstadoPermiteEntrar(ByVal Userindex As Integer, _
    ...
    Select Case guilds(GuildIndex).Alineacion
        ...
        Case ALINEACION_GUILD.ALINEACION_ARMADA
            m_EstadoPermiteEntrar = (UserList(Userindex).Faccion.Bando = eFaccion.Armada) 'Not criminal(Userindex) And IIf(UserList(Userindex).Stats.ELV >= 25, UserList(Userindex).Faccion.ArmadaReal <> 0, True)
        
        Case ALINEACION_GUILD.ALINEACION_LEGION
            m_EstadoPermiteEntrar = (UserList(Userindex).Faccion.Bando = eFaccion.Legion) 'criminal(Userindex) And IIf(UserList(Userindex).Stats.ELV >= 25, UserList(Userindex).Faccion.FuerzasCaos <> 0, True)
        
        Case ALINEACION_GUILD.ALINEACION_NEUTRO
            m_EstadoPermiteEntrar = (UserList(Userindex).Faccion.Bando = eFaccion.Neutral) 'UserList(Userindex).Faccion.ArmadaReal = 0 And UserList(Userindex).Faccion.FuerzasCaos = 0
