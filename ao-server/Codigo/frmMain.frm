...
Private Sub AutoSave_Timer()
   ...
   ' eventoM = eventoM + 1
    ...
    'If eventoM = 60 Then ' cada 60 segundos se llama al timer x minuto de los eventos x2
    Call mod_Eventos.PasaMinuto
    Call mod_Subasta.subasta_PasarMinuto
    If Torneo.Activo = True And Torneo.Comenzo = False Then
        If Torneo.autoCancelar > 0 Then Torneo.autoCancelar = Torneo.autoCancelar - 1
        If Torneo.autoCancelar <= 5 And Torneo.autoCancelar > 0 Then
            Call MensajeGlobal("Torneo> Hay cupos disponibles. El torneo se autocancelara en " & Torneo.autoCancelar & " minutos. Escribe /TORNEO para inscribirte", FontTypeNames.FONTTYPE_GUILD)
        End If
        If Torneo.autoCancelar = 0 Then
            cancelar_Torneo True
        End If
    End If
    '   eventoM = 1
   ' End If
