    ...
    If InMapBounds(Map, x, Y) Then
        ...
        With UserList(Userindex)
            ...
            If MapData(Map, x, Y).NpcIndex > 0 Then     'Acciones NPCs
                ...
                ElseIf Npclist(tempIndex).NPCtype = eNPCType.Viajante Then
                    Call writeViajesForm(Userindex)
                    
                ElseIf Npclist(tempIndex).NPCtype = eNPCType.Subastador Then
                    If Subasta.Activo = False Then _
                        Call writeAbrirSubasta(Userindex)
                    
                ElseIf Npclist(tempIndex).NPCtype = eNPCType.tCanjes Then
                    Call writeSendCanjes(Userindex)
