        ...
        If .Invent.EscudoEqpObjIndex > 0 Then
            ...
            If Not UsuarioImpacto Then
                ...
                If Rechazo Then
                    ...
                    Call SendData(SendTarget.ToPCArea, VictimaIndex, PrepareMessageCharacterDefendAnim(.Char.CharIndex))
...
Public Function PuedeAtacar(ByVal AttackerIndex As Integer, _
    ...
    If UserList(AttackerIndex).userTorneo.EnTorneo = True Then
        If Not torneo_PuedeAtacar(AttackerIndex, VictimIndex) Then
            PuedeAtacar = False
            Exit Function
        End If
    End If
    ...
    If esArmada(AttackerIndex) Or esArmada(VictimIndex) Then
        ' No puede
        Call WriteConsoleMsg(AttackerIndex, "Los miembros de la armada tienen prohibido atacarse entre si.", FontTypeNames.FONTTYPE_WARNING)
        Exit Function
    End If
    ...
    If esLegion(VictimIndex) Then
        If esLegion(AttackerIndex) Then
...
Public Function PuedeAtacarNPC(ByVal AttackerIndex As Integer, _
    ...
    With Npclist(NpcIndex)
        ...
        If MasterIndex > 0 Then
            ...
            If esArmada(MasterIndex) Then
                
                ' Atacante ciuda?
                If esArmada(AttackerIndex) Then
                        'El atacante es Armada y esta intentando atacar mascota de un Ciudadano
                        Call WriteConsoleMsg(AttackerIndex, "Los miembros del ejercito real no pueden atacar mascotas de miembros de su misma facci�n.", FontTypeNames.FONTTYPE_INFO)
                        Exit Function
                End If
            
                'Es mascota de un caos?
            ElseIf esLegion(MasterIndex) Then
        ...
        ElseIf .Owner > 0 Then
            ...
            ' Si no son del mismo clan o party, pueden atacar (No renueva el timer)
...
Sub CalcularDarExp(ByVal Userindex As Integer, _
    ...
    'SANTO:: Solo le da EXP si el nivel del npc es menor o igual al nivel del usuario.
    If (eventox2 = eEventoX2.Exp) And (mapasx2(x2CurrMap).num = UserList(Userindex).Pos.map) Then ExpaDar = ExpaDar * 2 'evento experiencia x2
    If (ExpaDar > 0) And (Npclist(NpcIndex).level <= UserList(Userindex).Stats.ELV) Then
...
Public Sub LanzarProyectil(ByVal Userindex As Integer, ByVal x As Byte, ByVal y As Byte)
    ...
    With UserList(Userindex)
        ...
        Call SendData(SendTarget.ToPCArea, Userindex, PrepareMessageCharacterAttackAnim(.Char.CharIndex))
