...
'***************************************************
    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Select Case Comando
            ...
            Case "/CREARTORNEO"
                If CantidadArgumentos = 2 Then
                    Call WriteAdminTorneo(1, ArgumentosAll(1), Val(ArgumentosAll(0)))
                Else
                    Call ShowConsoleMsg("Prámetros incorrectos. Utilice /CREARTORNEO NUMEQUIPOS(8,16,32 o 64) USERSXEQUIPO(1,2,3,4,5)")
                End If
                
            Case "/CANCELARTORNEO"
                Call WriteAdminTorneo(2, 0, 0)
                
            Case "/FACCION"
                frmFaccion.Show , frmMain
            Case "/MEDITAR"
            ...
            Case "/OFRECER"
                If notNullArguments Then
                    If ValidNumber(ArgumentosRaw, eNumber_Types.ent_Long) Then
                        Call writeSubastaOfrecer(CLng(ArgumentosRaw))
                    Else
                        'No es numerico
                            Call ShowConsoleMsg("Formato invalido. Escribe /ofrecer CANTIDAD")
