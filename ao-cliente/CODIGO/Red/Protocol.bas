...
On Error Resume Next
    ...
    Select Case Packet
        ...
        Case ServerPacketID.SendCanjes
            Call handleSendCanjes
            
        Case ServerPacketID.ViajesForm
            Call handleViajesForm
            
        Case ServerPacketID.AbrirSubasta
            Call handleAbrirSubasta
        ...
        Case ServerPacketID.BarraCasteo           ' MP, +, * and _ '
            Call HandleBarraCasteo
        ...
        Case ServerPacketID.PlayDefendAnim
            Call HandleDefendAnim
...
Private Sub WriteChatOverHeadInConsole(ByVal CharIndex As Integer, ByVal ChatText As String, ByVal Red As Byte, ByVal Green As Byte, ByVal Blue As Byte)
    ...
    With charlist(CharIndex)
        ...
        If .priv = 0 Then
            NameRed = coloresFaccion(.nickColor).r
            NameGreen = coloresFaccion(.nickColor).g
            NameBlue = coloresFaccion(.nickColor).b
...
Private Sub HandleDefendAnim()
    ...
    charlist(CharIndex).defending = True 'should be done in separated sub?
End Sub
...
Public Sub WriteAdminTorneo(ByVal accion As Byte, ByVal tipoTorneo As Byte, ByVal numUsers As Byte)
    With outgoingData
        Call .WriteByte(ClientPacketID.AdminTorneo)
        
        Call .WriteByte(accion)
        
        If accion = 1 Then
            Call .WriteByte(numUsers)
            Call .WriteByte(tipoTorneo)
        End If
...
Public Sub WriteDoDeath(ByVal Slots As Byte, ByVal Drop As Byte)
    With outgoingData
        Call .WriteByte(ClientPacketID.Do_Death)
...
Public Sub WriteRequestCanjes()
    Call outgoingData.WriteByte(ClientPacketID.requestCanjes)
End Sub

Public Sub writeCanjearItem(ByVal Index As Integer)
    Call outgoingData.WriteByte(ClientPacketID.CanjearItem)
    Call outgoingData.WriteInteger(Index)
End Sub

Private Sub handleAbrirSubasta()
    With incomingData
        Call .ReadByte
        frmIniciarSubasta.Show , frmMain
        
    End With
End Sub

Private Sub handleViajesForm()
    With incomingData
        Call .ReadByte
...
Private Sub handleSendCanjes()
    Dim i As Long
    With incomingData
        Call .ReadByte
        numCanjes = .ReadInteger
        ...
        For i = 1 To numCanjes
            With canjes(i)
                ...
                .oros = incomingData.ReadByte
                .platas = incomingData.ReadByte
                .bronces = incomingData.ReadByte
                
                .GrhIndex = incomingData.ReadInteger
                
                frmCanjes.List1.AddItem .Nombre
            End With
        Next i
    End With
End Sub

Public Sub WriteIniciarSubasta(ByVal slot As Byte, ByVal Amount As Integer, ByVal ofertainicial As Long)
    With outgoingData
        Call .WriteByte(ClientPacketID.IniciarSubasta)
        Call .WriteByte(slot)
        Call .WriteInteger(Amount)
        Call .WriteLong(ofertainicial)
    End With
End Sub

Public Sub writeSubastaOfrecer(ByVal oferta As Long)
    outgoingData.WriteByte ClientPacketID.SubastaOfrecer
    outgoingData.WriteLong oferta
End Sub
...
Public Sub writedoViajar(ByVal donde As Byte)
    Call outgoingData.WriteByte(ClientPacketID.doViajar)
    Call outgoingData.WriteByte(donde)
End Sub



Private Sub HandleBarraCasteo()
...
Public Sub WriteElegirFaccion(ByVal bando As Byte)
    Call outgoingData.WriteByte(ClientPacketID.ElegirFaccion)
    Call outgoingData.WriteByte(bando)
End Sub
