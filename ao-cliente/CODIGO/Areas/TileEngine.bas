...
Private Sub CharRender(ByVal CharIndex As Long, ByVal PixelOffsetX As Integer, ByVal PixelOffsetY As Integer)
    ...
    With charlist(CharIndex)
        ...
        'if is defending we set the attack anim
        If .defending And .Arma.WeaponWalk(.Heading).Started = 0 Then
            .Escudo.ShieldWalk(.Heading).Started = 1
            .Escudo.ShieldWalk(.Heading).FrameCounter = 1
        
            'if the anim has ended or we are no longer attacking end the animation
        ElseIf .Escudo.ShieldWalk(.Heading).FrameCounter > 4 And .defending Then
            .defending = False 'this is just for testing, it shouldnt be done here
        End If
        ...
        If Not moved Then
            ...
            If Not .Movement And Not .defending Then
        ...
        Else
            ...
            Else
                ...
                If .nickColor = eNickColor.ieCriminal Or .nickColor = eNickColor.ieLegion Then
                    Call Engine_Long_To_RGB_List(ColorFinal(), D3DColorARGB(100, 255, 100, 100))
                ElseIf .nickColor = eNickColor.ieCiudadano Or .nickColor = eNickColor.ieArmada Then
        ...
        If Not .invisible Then
            ...
            If .max > 0 Then
                'primero dibujamos la barra blanca completa, despues la azul mas angosta y corta del tamano del progreso.
                
                'Call Draw_FilledBox(PixelOffsetX - 35, PixelOffsetY, (((.min / 100) / (.max / 100)) * 70), 5, D3DColorARGB(200, 255, 0, 0), D3DColorARGB(200, 0, 0, 0))
               ' Call Draw_FilledBox(PixelOffsetX - 35, PixelOffsetY, (((.min / 100) / (.max / 100)) * 70), 5, D3DColorARGB(255, 0, 0, 255), D3DColorARGB(200, 128, 128, 128))
                
                If .min < .max Then
                    Engine_Draw_Box PixelOffsetX - 10, PixelOffsetY - 30, (((.min / 100) / (.max / 100)) * 50), 5, D3DColorARGB(255, 155, 155, 255)
                    Engine_Draw_Box PixelOffsetX - 10, PixelOffsetY - 30, 50, 5, D3DColorARGB(150, 0, 0, 165)
                    .min = .min + timerElapsedTime
...
Private Sub RenderName(ByVal CharIndex As Long, _
    ...
    With charlist(CharIndex)
        ...
        If .priv = 0 Then
            If .muerto Then
                Color = D3DColorARGB(188, coloresFaccion(.nickColor).r, coloresFaccion(.nickColor).g, coloresFaccion(.nickColor).b)
            Else
                Color = D3DColorARGB(255, coloresFaccion(.nickColor).r, coloresFaccion(.nickColor).g, coloresFaccion(.nickColor).b)
               ' Debug.Print .nickColor & "   " & .Nombre
             '   If .Criminal Then
              '      Color = ColoresPJ(50)
              '  Else
             '       Color = ColoresPJ(49)
