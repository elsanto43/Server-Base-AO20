...
Sub ClickEdit(Button As Integer, tX As Integer, tY As Integer)
    ...
    If Button = vbLeftButton Then
        ...
        If frmMain.cInsertarLuz.Value Then
            ...
            MapData(tX, tY).light_index = Create_Light_To_Map(tX, tY, frmMain.cRango, Val(frmMain.R), Val(frmMain.G), Val(frmMain.B))
            MapInfo.Changed = 1 'Set changed flag
        ElseIf frmMain.cQuitarLuz.Value Then
            If MapData(tX, tY).light_index > 0 Then
                Delete_Light_To_Map tX, tY ' MapData(tX, tY).light_index
                MapData(tX, tY).light_index = 0
                MapInfo.Changed = 1 'Set changed flag
            End If
        End If
    End If
